...
Below is a high-level overview of the development roadmap for the OHDSI Methods Library. Note that each package maintains a much more detailed list of issues.
...
- Add the `PheValuator` package to the Methods Library. 
- Add the `CohortDiagnostics` package to the Methods Library. 

# Medium term

These task will be taken on after the short term tasks are complete. We expect to finish them within the next year:

- Add the `ROhdsiWebApi` package to the Methods Library. 
- Add the `DataQualityDashboard` package to the Methods Library.
- Add the `OhdsiSharing` package to the Methods Library.  
- Add support for CDM version 6 to all packages in the Methods Library.
...
- Move all Methods Library packages to CRAN.
